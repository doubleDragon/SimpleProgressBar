package com.wsl.library.progress;
...
public class ProgressWheel2 extends View {
    ...
    private int secondTextColor = 0xFF000000;
    ...
    private Paint textPercentPaint = new Paint();
    private Paint secondTextPaint = new Paint();
    ...
    private void setupPaints() {
        ...
        textPercentPaint.setColor(textColor);
        textPercentPaint.setStyle(Style.FILL);
        textPercentPaint.setAntiAlias(true);
        textPercentPaint.setTextSize(sp2px(9));
        textPercentPaint.setTextAlign(Paint.Align.LEFT);

        secondTextPaint.setColor(secondTextColor);
        secondTextPaint.setStyle(Style.FILL);
        secondTextPaint.setAntiAlias(true);
        secondTextPaint.setTextSize(sp2px(9));
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressWheel2_progressBarWidth,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressWheel2_rimWidth,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.ProgressWheel2_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.ProgressWheel2_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.ProgressWheel2_barColor, barColor);

        barLength = (int) a.getDimension(R.styleable.ProgressWheel2_progressBarLength,
                barLength);

        textSize = (int) a.getDimension(R.styleable.ProgressWheel2_textSize,
                textSize);

        textColor = (int) a.getColor(R.styleable.ProgressWheel2_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.ProgressWheel2_text)) {
            setText(a.getString(R.styleable.ProgressWheel2_text));
        }

        secondTextSize = (int) a.getDimension(R.styleable.ProgressWheel2_secondTextSize,
                secondTextSize);

        secondTextColor = (int) a.getColor(R.styleable.ProgressWheel2_secondTextColor,
                secondTextColor);

        //if the second text is empty , so ignore it
        if (a.hasValue(R.styleable.ProgressWheel2_secondText)) {
            setSecondText(a.getString(R.styleable.ProgressWheel2_secondText));
        }

        rimColor = (int) a.getColor(R.styleable.ProgressWheel2_rimColor,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.ProgressWheel2_circleColor,
                circleColor);

        contourColor = a.getColor(R.styleable.ProgressWheel2_contourColor, contourColor);
        contourSize = a.getDimension(R.styleable.ProgressWheel2_contourSize, contourSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float textPercentWidth = textPercentPaint.measureText(textPercent);

        float horizontalTextOffset = (textWidth1 - textPercentWidth) / 2;
        canvas.drawText(text, this.getWidth() / 2 + horizontalTextOffset,
                getHeight() / 2, textPaint);

        //draw %
        canvas.drawText(textPercent, this.getWidth() / 2 + horizontalTextOffset,
                getHeight() / 2, textPercentPaint);

        //draw second text
        float textHeight2 = secondTextPaint.descent() - secondTextPaint.ascent();
        canvas.drawText(secondText, this.getWidth() / 2,
                getHeight() / 2 + textHeight2, secondTextPaint);
        if (isSpinning) {
            scheduleRedraw();
        }
    }
